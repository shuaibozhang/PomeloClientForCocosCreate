...
function pomelo_require(path, parent, orig) {
  ...
  if (!module.exports) {
    ...
    module.call(window, module.exports, pomelo_require.relative(resolved), module);
  }
...
pomelo_require.resolve = function(path) {
  ...
  for (var i = 0; i < paths.length; i++) {
    var path = paths[i];
    if (has.call(pomelo_require.modules, path)) return path;
  }
...
pomelo_require.relative = function(parent) {
  ...
  localRequire.resolve = function(path) {
    ...
    if ('.' == c) return pomelo_require.normalize(p, path);
...
var index = pomelo_require('component-indexof');
...
pomelo_require.register("boot/index.js", function(exports, require, module){
  var Emitter = pomelo_require('component-emitter');
  window.EventEmitter = Emitter;

  var protocol = pomelo_require('NetEase-pomelo-protocol');
  window.Protocol = protocol;

  var protobuf = pomelo_require('pomelonode-pomelo-protobuf');
  window.protobuf = protobuf;

  var pomelo = pomelo_require('pomelonode-pomelo-jsclient-websocket');
  window.pomelo = pomelo;

  console.log("pomelo require finish");
});
